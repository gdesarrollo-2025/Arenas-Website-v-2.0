...
export default function SearchSection ({filtersPanel, children}: props){
    return <section className="relative flex max-w-[90vw] w-full mx-auto border-2">
        <aside className="w-fit lg:w-1/3 xl:w-1/4 ">
            <div className="hidden lg:block w-full px-4 py-6  h-fit shadow-md/70 rounded-[20px]">
                {filtersPanel}
            </div>
            <Sheet>
                <SheetTrigger render={<button className="sticky flex items-center justify-center top-20 lg:hidden bg-accent size-10 aspect-square rounded-lg"><FaGears className="text-white size-8"/></button>}/>
